// Metrics Bot Function App Module
// This module creates a Function App for the Metrics Bot
    ...
    siteConfig: {
      ...
      appSettings: [
        ...
        }
        {
          ...
          value: toLower('metricsbot-${environmentName}')
        }
        {
        ...
        }
        {
          ...
          value: !empty(keyVaultUri) ? '@Microsoft.KeyVault(SecretUri=${keyVaultUri}secrets/OtelExporterEndpoint)' : ''
        }
        {
...
resource privateEndpoint 'Microsoft.Network/privateEndpoints@2022-07-01' = if (networkingConfig.privateEndpoints.enabled && !empty(networkingConfig.subnetId)) {
  name: '${baseName}-pe-vv-${projectName}-metricsbot'
  location: location
  tags: tags
  properties: {
    privateLinkServiceConnections: [
      {
        name: 'metricsbot-function-connection'
        properties: {
          privateLinkServiceId: metricsBotFunction.id
          groupIds: [
            'sites'
          ]
        }
...
// Outputs
output functionAppId string = metricsBotFunction.id
output functionAppName string = metricsBotFunction.name
output functionAppPrincipalId string = metricsBotFunction.identity.principalId
output functionAppHostName string = metricsBotFunction.properties.defaultHostName